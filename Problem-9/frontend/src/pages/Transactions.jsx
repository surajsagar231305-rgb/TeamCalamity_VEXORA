...
const PAYMENT_METHODS = ['All', 'UPI', 'Credit Card', 'Debit Card', 'Net Banking', 'Bank Transfer', 'Cash', 'Other'];
...
export const Transactions = () => {
  ...
  }, [search, typeFilter, categoryFilter, accountFilter, paymentMethodFilter, startDate, endDate, sortIndex, page]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            Search, filter, categorize, and export your entire financial history.
          </p>
        </div>
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs overflow-hidden">
        {loading ? (
          ...
          <div className="py-16">
            <EmptyState
              title={activeFilterCount > 0 ? "No matching transactions found" : "No transactions recorded yet"}
              description={activeFilterCount > 0 ? "Try adjusting your search query, type, or date range filters." : "Start by adding your first transaction."}
              actionText={activeFilterCount > 0 ? "Reset Filters" : "Add Transaction"}
              onAction={activeFilterCount > 0 ? handleClearFilters : () => setModalOpen(true)}
            />
          </div>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead className="text-[11px] font-bold uppercase tracking-wider text-slate-400 bg-slate-50/60 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="py-3.5">Title & Description</th>
