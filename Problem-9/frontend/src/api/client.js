...
export const api = {
  dashboard: {
    getSummary: (period = 'this_month') => request(`/api/dashboard/summary${toQueryString({ period })}`),
    getCategoryExpenses: (period = 'this_month') => request(`/api/dashboard/category-expenses${toQueryString({ period })}`),
    getMonthlySummary: (months = 6) => request(`/api/dashboard/monthly-summary${toQueryString({ months })}`),
    getTrend: (period = 'this_month') => request(`/api/dashboard/trend${toQueryString({ period })}`),
    getInsights: () => request('/api/dashboard/insights'),
  },
  transactions: {
    list: (params = {}) => request(`/api/transactions${toQueryString(params)}`),
    ...
    getExportUrl: (params = {}) => `${API_BASE}/api/transactions/export/csv${toQueryString(params)}`,
    uploadReceipt: async (file) => {
      const formData = new FormData();
      formData.append('file', file);
      const res = await fetch(`${API_BASE}/api/transactions/upload-receipt`, {
        method: 'POST',
        body: formData,
      });
    ...
    scanInvoice: async (file) => {
      const formData = new FormData();
      formData.append('file', file);
      const res = await fetch(`${API_BASE}/api/transactions/scan-invoice`, {
        method: 'POST',
        body: formData,
      });
      if (!res.ok) {
  ...
  budgets: {
    list: (month, year) => request(`/api/budgets${toQueryString({ month, year })}`),
    create: (data) => request('/api/budgets', { method: 'POST', body: JSON.stringify(data) }),
    update: (id, data) => request(`/api/budgets/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  ...
  creditCards: {
    ...
    toggleStatus: (id) => request(`/api/credit-cards/${id}/toggle-status`, { method: 'PATCH' }),
  },
  insurance: {
